...
function withCanvasEventRegistration(Cevent) {
    "use strict";
    let math = Math, PI = math.PI, TWOPI = 2 * PI, DEGREE = PI / 180, sqrt = math.sqrt, pow = math.pow, cos = math.cos,
        ...
        distance = function (p1, p2) {
            ...
            let deltaX = x2 - x1, deltaY = y2 - y1, closestPoint = {}, u;
            ...
            init: function (x, y) {
                ...
                let p = rotate(this.x * this.scaleX, this.y * this.scaleY, this.rotation);
                ...
                let shadowBlur = this.shadowBlur, shadowOffsetX = this.shadowOffsetX, shadowOffsetY = this.shadowOffsetY;
                ...
                let zoom = Cevent.__zoom, scaleX = this.scaleX * zoom, scaleY = this.scaleY * zoom,
            ...
            init: function (x, y, width, height, radius) {
                ...
                let thisPos = this.position(), mousePos = rotate(point.x - thisPos.x, point.y - thisPos.y, -this.rotation);
            ...
            draw: function (ctx) {
                let x = this.x, y = this.y, w = this.w, h = this.h, C = .5522847498307933, c_x = C * w, c_y = C * h;
                this.applyStyle(ctx);
                this.setTransform(ctx);
                ctx.beginPath();
                ctx.moveTo(x + w, y);
            ...
            init: function (x, y, radius) {
                ...
                let lineWidth = !!this.stroke && this.lineWidth, thisPos = this.position();
        ...
        NUMBER = /[\-+]?(?:\d+[.]?\d*|[.]\d+)(?:[Ee][\-+]?\d+)?/g, angleBetweenVectors = function (x1, y1, x2, y2) {
            let dotproduct = x1 * x2 + y1 * y2, d1 = sqrt(x1 * x1 + y1 * y1), d2 = sqrt(x2 * x2 + y2 * y2),
            ...
            let i, cx = currentX, cy = currentY, l = arguments.length;
            check(arguments, 0, 2, 2);
            checkcurrent();
            for (i = 0; i < l; i += 2) {
            ...
            let i, a = arguments, l = arguments.length;
            check(a, 0, 6, 6);
            ensure(this, cx1, cx2);
            ...
            let i, a = arguments, l = a.length, x0 = currentX, y0 = currentY;
            check(a, 0, 6, 6);
            checkcurrent();
            for (i = 0; i < l; i += 6) {
            ...
            let i, a = arguments, l = a.length, x0 = currentX, y0 = currentY, cx0 = _lastCCP[0], cy0 = _lastCCP[1], cx1,
                cx2, cy1, cy2, x, y;
            check(arguments, 0, 4, 4);
            checkcurrent();
            for (i = 0; i < l; i += 4) {
            ...
            let i, a = arguments, l = arguments.length, x0 = currentX, y0 = currentY, cx0 = _lastQCP[0], cy0 = _lastQCP[1],
                cx, cy, x, y;
            check(arguments, 0, 2, 2);
            checkcurrent();
            for (i = 0; i < l; i += 2) {
            ...
            let i, a = arguments, l = a.length, x0 = currentX, y0 = currentY, cx0 = _lastQCP[0], cy0 = _lastQCP[1], cx, cy,
                x, y;
            check(arguments, 0, 2, 2);
            checkcurrent();
            for (i = 0; i < l; i += 2) {
            ...
            clockwise = !!clockwise;
            checkcurrent();
            let x1 = currentX, y1 = currentY, x2 = x, y2 = y, phi = rotation * DEGREE, sinphi = sin(phi), cosphi = cos(phi),
            ...
            if (lambda > 1) {
                ...
                let rxrx = rx * rx, ryry = ry * ry, x1x1$ = x1$ * x1$, y1y1$ = y1$ * y1$, t = rxrx * y1y1$ + ryry * x1x1$;
            ...
            let sp = rotatePoint(rx * cos(sa), ry * sin(sa), rotation), sx = cx + sp[0], sy = cy + sp[1],
            ...
            if (l % 2 === 0) {
                ...
                let radius = a[l - 1], n = (l - 1) / 2, x0 = (a[n * 2 - 2] + a[0]) / 2, y0 = (a[n * 2 - 1] + a[1]) / 2,
                    temp_x, temp_y;
            ...
            let matches = svg.match(SVGPATTERN), match, parts, args, i, j, path = [];
            if (!matches) {
...
function createCanvasEvent(window, doc, undefined) {
    "use strict";
    let toString = Object.prototype.toString, func = "function", string = "string", array = "array", object = "object",
        ...
        normalizeEvent = function (e) {
            ...
            let touchevent = eventsMap[type];
            if (!touchevent) return callback;
            return function (event) {
                let isTouchEvent = event.type === touchevent;
            ...
            let shapeHandlers = data(shape, eventType), globalHandlers = data(self.cv, eventType);
            ...
            let shape, shapes = self._shapes, i = shapes.length;
            ...
            return function (e) {
                ...
                let x = ((e && e.pageX - self._pos.x + 1 || window.event.offsetX + 1) - 1) / self.__zoom || 0;
                let y = ((e && e.pageY - self._pos.y + 1 || window.event.offsetY + 1) - 1) / self.__zoom || 0;
                if (!TOUCH_MOVE && Math.abs(x - self.x) < 2 && Math.abs(y - self.y) < 2) return;
                self.x = x;
                self.y = y;
                TOUCH_MOVE = true;
                if (!self._clicked && USE_MOUSE_OVER_OUT) {
                    let shapeUnderCursor = getObjectUnderCursor(e, self);
                    if (shapeUnderCursor) {
            ...
            return function (e) {
                ...
                self._curHover;
                let curHover = self._curHover, which = mouseButtons[e.which || e.button];
                self._clicked = true;
            ...
            return function (type, self) {
                let eventObj = data(self.cv, type, {}), Handlers = data(eventObj);
                return function (e) {
                    if (currentCanvas !== self.cv && !self.__globalkeyevents) {
                        return
                    }
                    code = type == "keydown" ? e.keyCode : code;
                    let special = hotkeys.specialKeys[code],
                    ...
                    if (handlers) {
                        let ret = handleEvent(self, handlers, self, e);
                        if (!self.play) {
                            self.redraw()
                        }
                        return ret
    ...
    Cevent.fn = Cevent.prototype = {
        init: function (cv, shapes) {
            this.cv = cv;
            if (window.getComputedStyle) this.background = getComputedStyle(this.cv.parentElement).backgroundColor; else this.background = "#152937";
            ...
            let ret = [], match = SELECTOR.exec(selector), type, name;
            if (selector === "*") {
            ...
            let match = SELECTOR.exec(id), shapes = this._last;
            ...
            let shapes = obj || this._last, type;
            ...
            let shape, i = 0, shapes = this._shapes;
            this.ctx.save();
    ...
    function makeLive(selector, fn) {
        let match = SELECTOR.exec(selector);
    ...
    Cevent.fn.drag = function (handlers) {
        let start, move, end, afterMove, self, objs = [], selector, dragid = "Cevent-drag" + hash, shapes = this._last;
        ...
        let a = Cevent(this.cv, objs).mousedown(mousedown).mouseup(mouseup).mousemove(mousemove);
    ...
    Cevent.registre = Cevent.register = function (name, Class) {
        ...
        let constName = name.charAt(0).toUpperCase() + name.substring(1);
        this[constName] = Class;
        this.prototype[name] = function () {
            let shape = Class.apply(this, arguments);
            shape[""] = name;
...
export const Cevent = withCanvasEventRegistration(createCanvasEvent(window, document))
